...
var gestureup = supportsTouch ? 'touchend' : 'mouseup';
var gesturedown = supportsTouch ? 'touchstart' : 'mousedown';
var gesturemove = supportsTouch ? 'touchmove' : 'mousemove';
    ...
    c = h.drag = {
        ...
        setup: function() {
            if (!f.data(this, c.datakey)) {
                ...
                i.add(this, gesturedown, c.init, b);
        ...
        teardown: function() {
            if (!f.data(this, c.datakey).related) {
                ...
                i.remove(this, gesturedown, c.init);
        ...
        init: function(b) {
            ...
            if (! (a.which > 0 && b.which != a.which)) if (!f(b.target).is(a.not)) if (! (a.handle && !f(b.target).closest(a.handle, b.currentTarget).length)) {
                ...
                if (a.propagates) {
                    ...
                    c.textselect(false);
                    i.add(document, [gesturemove, gestureup].join(' '), c.handler, a);
                    return false;
                }
            }
        },
        ...
        handler: function(b) {
            if(supportsTouch){
                rlog('Received event of type ' + b.type);
            }
            var a = b.data;
            switch (b.type) {
            case ! a.dragging && (gesturemove): if (Math.pow(b.pageX - a.pageX, 2) + Math.pow(b.pageY - a.pageY, 2) < Math.pow(a.distance, 2)) break;
                b.target = a.target;
            ...
            case gestureup:
                i.remove(document, [gesturemove, gestureup].join(' '), c.handler);
                if (a.dragging) {
                    a.drop !== false && h.drop && h.drop.handler(b, a);
