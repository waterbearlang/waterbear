...
(function(SAT) {
   ...
   */
  Vector.prototype.rotate = function(deg, point) {
    ...
    this.x = parseInt(dx2 + x1);
    this.y = parseInt(dy2 + y1);
  ...
  Polygon.prototype.rotate = function(deg, about){
    about = typeof about !== 'undefined' ? a : this.average;
    var points = this.points;
    var len = points.length; 
    for (var i = 0; i < len; i++) {
      points[i].rotate(degs, about);
    }
  };
  ...
  Polygon.prototype.average = function(){
    ...
    for (var i = 0; i < len-1; i++) {
      average = points[i].add(points[i+1])
    }
    average.x /= len;
    average.y /= len;
  };
