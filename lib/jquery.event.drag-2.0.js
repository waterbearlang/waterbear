...
var move_event = supportsTouch ? 'touchmove' : 'mousemove';
var down_event = supportsTouch ? 'touchstart' : 'mousedown';
var up_event = supportsTouch ? 'touchend' : 'mouseup';
...
drag = $special.drag = {
	...
	setup: function(){
		...
		$event.add( this, down_event, drag.init, data );
	...
	teardown: function(){
		...
		$event.remove( this, down_event, drag.init );
	...
	init: function( event ){
		...
		$event.add( document, move_event + ' ' + up_event, drag.handler, dd );
		...
		switch ( event.type ){
			...
			case move_event: 
				if ( dd.dragging ){
					...
					event.type = up_event; // helps "drop" handler behave
				}
			// mouseup, stop dragging
			case up_event: 
				$event.remove( document, move_event + ' ' + up_event, drag.handler ); // remove page events
				if ( dd.dragging ){
					if ( dd.drop !== false && $special.drop ) 
