...
(function(SAT) {
  ...
  function convertSidelengthUnits(x, unit) {
    switch(unit) {
      case "px":
        alert("px");
        return x;
   ...
   * @param {?unit=} wUnit Optional param, specifices units of w (px/pt/em/%)
   * @param {?unit=} hUnit Optional param, specifices units of w (px/pt/em/%)
   * @constructor
   */
  var Box = function(pos, w, h, wUnit, hUnit) {
    this['pos'] = this.pos = pos || new Vector();
    this['w'] = this.w = convertSidelengthUnits(w, wUnit) || 0; 
    this['h'] = this.h = convertSidelengthUnits(h, hUnit) || 0; 
  };
